...
use crate::repositories::{AdminRepository, UserRepository};
...
async fn handle_login(
    ...
    if password_hash.is_empty() {
        // Autoriser la connexion sans vérification de mot de passe si mdp vide
        log::warn!(
...
pub async fn register(pool: web::Data<MySqlPool>, req: web::Json<RegisterRequest>) -> HttpResponse {
    ...
    match user_repo
        .create(&req.email, &password_hash, &req.first_name, &req.last_name)
        .await
    {
        Ok(user_id) => {
            let secret = std::env::var("JWT_SECRET").unwrap();
...
pub async fn get_profile(pool: web::Data<MySqlPool>, claims: web::ReqData<Claims>) -> HttpResponse {
    let user_repo = UserRepository::new(pool.get_ref().clone());
